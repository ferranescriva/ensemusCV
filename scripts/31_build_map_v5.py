...
df_hmd = pd.read_csv(os.path.join(PROC_DIR, "censo_hmd_cv_20260522.csv"), sep=';')
df_hmd['lat'] = pd.to_numeric(df_hmd['lat'], errors='coerce')
df_hmd['lon'] = pd.to_numeric(df_hmd['lon'], errors='coerce')
df_hmd = df_hmd.dropna(subset=['lat','lon'])
...
for feat in cobertura['features']:
    ...
    if tipus == 'cobertura_60min':
        folium.GeoJson(feat, style_function=lambda x: {
            'fillColor': '#AED6F1', 'color': '#2E86C1',
            'weight': 1, 'fillOpacity': 0.25,
        }, tooltip="Cobertura 60 min en vehicle").add_to(cob_60)
    elif tipus == 'cobertura_30min':
        folium.GeoJson(feat, style_function=lambda x: {
            'fillColor': '#2E8B57', 'color': '#1a5c35',
            'weight': 1, 'fillOpacity': 0.30,
        }, tooltip="Cobertura 30 min en vehicle").add_to(cob_30)

cob_60.add_to(m)
cob_30.add_to(m)

grup_hmd = folium.FeatureGroup(name="IES amb Batxillerat Arts (Musica)", show=False)

for _, row in df_hmd.iterrows():
    nom     = str(row.get('denominacion',''))
    munic   = str(row.get('municipio',''))
    comarca = str(row.get('comarca',''))
    prov    = str(row.get('provincia',''))
    codi    = str(row.get('codigo',''))

    popup = "<div style='font-family:Arial;min-width:220px;font-size:12px'>"
    popup += "<b style='font-size:13px'>" + nom + "</b><br>"
    popup += "<span style='background:#FFF8DC;padding:1px 4px;border-radius:3px;font-size:10px'>Batxillerat d'Arts - Via Musica</span><br>"
    popup += "<hr style='margin:4px 0'/>"
    popup += "<b>Municipi:</b> " + munic + "<br>"
    popup += "<b>Comarca:</b> " + comarca + "<br>"
    popup += "<b>Provincia:</b> " + prov + "<br>"
    popup += "<b>Codi:</b> " + codi + "<br>"
    popup += "<span style='color:#888;font-size:10px'>Font: Guia de Centres GVA (22/05/2026)</span>"
    popup += "</div>"
...
titol += "509 centres - Cobertura reglada 30/60 min - Batxillerat d'Arts</span></div>"
...
llegenda += "<span style='color:#FFD700;text-shadow:-1px 0 #8B4513, 0 1px #8B4513'>&#9650;</span> IES Batxillerat d'Arts (Musica)<br>"
...
m.save(out)
print("Mapa v5 guardat: " + out)
print("IES Batxillerat Arts: " + str(len(df_hmd)))